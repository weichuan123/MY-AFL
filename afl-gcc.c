...
static void edit_params(u32 argc, char** argv) {
  ...
  if (!strncmp(name, "afl-clang", 9)) {
    ...
    if (!strcmp(name, "afl-g++")) {
      ...
      cc_params[cc_par_cnt++] = "-includePolar/polar.hpp";
      cc_params[cc_par_cnt++] = "-D__POLAR_INS(_LOC)=do{__afl_area_ptr[_LOC]++;} while (0)";
      ...
      cc_params[cc_par_cnt++] = "-D__POLAR_INS(_LOC)=do{extern char* __afl_area_ptr;__afl_area_ptr[_LOC]++;} while (0)";
    }
...
int main(int argc, char** argv) {
  ...
  printf("%s ", cc_params[0]);
  int a=1;
  while(a<cc_par_cnt)
  {
    printf("%s\t",cc_params[a++]);
  }
  puts("");
